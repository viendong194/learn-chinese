...
export default function FillBlank({ data, onComplete }) {
  ...
  return (
    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
      <div className="flex flex-wrap items-center gap-2 text-lg mb-4">
        ...
        <span className={`border-b-2 px-4 py-1 min-w-[80px] text-center transition-colors ${isCheck
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {data.choices.map((choice, index) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl border-2 transition-all ${input === choice
      ...
      {!isCheck ? (
        <button
          onClick={checkAnswer}
          disabled={!input}
          className={`mt-6 px-6 py-2 rounded-lg text-white transition-all ${input ? "bg-orange-500 hover:bg-orange-600" : "bg-gray-300 cursor-not-allowed"
            }`}
        >
          Kiểm tra
        </button>
